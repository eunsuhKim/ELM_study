#%%
from elm_argon_class_scaled_alpha_iz import elm
import numpy as onp
import time
...
xs = onp.linspace(xl, xr, nx)
# xs = onp.logspace(np.log10(xl), np.logspace(xr), N_colloc)
# ts = onp.random.uniform(tl,tr,N_colloc)
roots= roots_legendre(nt-2)[0]#.reshape(-1,1)
ts_ = (roots+1)/2*(tr-tl)+tl
ts = np.concatenate([np.array([tl]),ts_,np.array([tr])])
# ts = onp.logspace(tl,tr,N_colloc)
...
#%%
plt.scatter(X_colloc[1,:],X_colloc[0,:])
#%%
...
#%%
t_val = 1e-9
scaling_coeff = [1e16,1e16,1e5,1e20,1e20]
idx = int(nx*nt*t_val/tr)
plt.figure(figsize=(20,8))
titles = ['$n_i$','$n_e$','-E','$\\Gamma_i$','$\\Gamma_e$']
for i in range(2):
    plt.subplot(1,2,i+1)
    plt.title(titles[i])
    plt.plot(xs_test,scaling_coeff[i]*U_pred[idx:idx+nx,i],label='pred')
