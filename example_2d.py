...
if is_py:
    ...
    sys.stdout = open(f"logs/2d_reg_result_method_{opt_num}.txt",'w')
...
model = elm(x= X_test, y=U_test, C = options[opt_num]['C'],
                hidden_units=32, activation_function='sin',
...
plt.yticks(fontsize=12)
# plt.show()
plt.savefig("figure/2d_"+model.elm_type+f"_result_method_{opt_num}.pdf")
...
for i in range(0,len(t_snap)):
    ...
    plt.ylim([0,1.05])
# plt.show()
plt.savefig("figure/2d_"+model.elm_type+f"_result_method_{opt_num}_(snapshot).pdf")
