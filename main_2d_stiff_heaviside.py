...
def U(X):
    ...
    return np.heaviside(x,1)*x**2+(1-np.heaviside(x,1)*(-1))\
        - np.heaviside(t,1)*t**2
U_test =U(X_test)
...
sys.stdout = open(f"logs/2d_stiff_heavi_reg_result_method_{opt_num}_{model.activation_function}.txt",'w')
...
plt.contourf(Xs, Ts,prediction.reshape(nt,nx), 100,vmin=np.min(U_test), vmax=np.max(U_test))
plt.colorbar()
...
plt.yticks(fontsize=12)
# plt.show()
plt.savefig("figure/2d_stiff_heavi_"+model.elm_type+f"_result_method_{opt_num}_{model.activation_function}.pdf")
...
# plt.show()
plt.savefig("figure/2d_stiff_heavi_"+model.elm_type+f"_result_method_{opt_num}_{model.activation_function}_(snapshot).pdf")
